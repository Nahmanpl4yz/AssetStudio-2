...
{
    var bytes = new byte[image.Width * image.Height * System.Runtime.InteropServices.Marshal.SizeOf<TPixel>()];
    image.ProcessPixelRows(accessor =>
    {
        int pixelSize = System.Runtime.InteropServices.Marshal.SizeOf<TPixel>();
        for (int y = 0; y < accessor.Height; y++)
        {
            var row = accessor.GetRowSpan(y);
            var rowBytes = MemoryMarshal.AsBytes(row);
            rowBytes.CopyTo(bytes.AsSpan(y * accessor.Width * pixelSize, accessor.Width * pixelSize));
        }
    });
    return bytes;
}
