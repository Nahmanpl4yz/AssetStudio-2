...
namespace AssetStudio
{
    ...
    {
        /// <summary>
        /// AssetStudio 2: global decoder mode toggle, set from AssetStudioGUIForm's Options menu
        /// and persisted in Properties.Settings.Default.decoderMode. "Better" routes BC1/BC2(DXT3)/
        /// BC3/BC4/BC5 through the managed BetterBCnDecoder (see that file for why); "Original"
        /// preserves the exact behaviour of stock AssetStudio (native decoders, DXT3 unsupported).
        /// </summary>
        public static DecoderMode Mode = DecoderMode.Better;
        ...
        private bool DecodeDXT1(byte[] image_data, byte[] buff)
        {
            if (Mode == DecoderMode.Better)
                return BetterBCnDecoder.DecodeBC1(image_data, m_Width, m_Height, buff);
        ...
        {
            if (Mode == DecoderMode.Better)
                return BetterBCnDecoder.DecodeBC3(image_data, m_Width, m_Height, buff);
        ...
        {
            if (Mode == DecoderMode.Better)
                return BetterBCnDecoder.DecodeBC4(image_data, m_Width, m_Height, buff);
        ...
        {
            if (Mode == DecoderMode.Better)
                return BetterBCnDecoder.DecodeBC5(image_data, m_Width, m_Height, buff);
